...
Both reasoning nodes call through `FallbackLLM` in [llm.py](backend/agents/llm.py), which tries `claude-haiku-4-5-20251001` first and falls back to `gemini-3.5-flash` on *any* exception, raising only if every provider fails. Gemini is skipped silently when `GEMINI_API_KEY` is unset, so the fallback is optional. Don't reintroduce a bare `ChatAnthropic` in a node — a live Anthropic 529 took the whole pipeline down before this existed. The paper scout uses plain `requests` against OpenAlex.

Two pins are load-bearing and must stay below their major bumps: `langchain-anthropic==0.1.23` (`<0.2`) and `langchain-google-genai==1.0.10` (`<2.0`). Later releases of either require `langchain-core>=0.3`, which conflicts with the pinned `langgraph==0.1.4` / `langchain==0.2.5`. The 2024-era Google client does reach current Gemini models, but give it explicit `timeout` and `max_retries` — its defaults retry silently for minutes on an unreachable model and read as a hang. `tavily-python` is unrelated to that ceiling and pinned to `0.8.0`; it must stay `>=0.7` because [web_scout.py](backend/agents/web_scout.py) passes `search_depth="fast"`, which older clients reject.
